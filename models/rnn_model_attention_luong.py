    ...
    def _build_graph(self):
        ...
        with tf.device('/gpu:0'):
            ...
            with tf.variable_scope("rnn-attention") as scope:
                # Train - encoder
                lstm_cell = tf.contrib.rnn.LSTMCell(self.rnn_size)
                ...
                Watth = tf.get_variable("Wh", [self.rnn_size, self.rnn_size], tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
                Wattb = tf.get_variable("Wb", [self.rnn_size, 1], tf.float32,
                                       initializer=tf.contrib.layers.xavier_initializer())
                Wcontext = tf.get_variable("Wc", [self.rnn_size * 2, self.rnn_size], tf.float32,
                                     initializer=tf.contrib.layers.xavier_initializer())

                # Select outputs
                outputs_last_h = tf.stack(encoder_state, axis = 0)[-1,:,:]
                ...
                score = tf.matmul(ht, tf.reshape(Wattb, [self.rnn_size,1]))
                ...
                context = tf.reduce_sum(outputs_rest_h * tf.reshape(alphas, [-1,self.max_sequence,1]), 1)
