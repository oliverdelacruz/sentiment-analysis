...
class RNN_Model(BasicModel):
    ...
    def _build_graph(self):
        ...
        with tf.device('/gpu:0'):
            ...
            emb_pad = tf.Variable(tf.zeros([1, self.embedding_dim], dtype=tf.float32), trainable=False)
            emb_train = tf.Variable(tf.random_uniform([3, self.embedding_dim], -0.1, 0.1))
            emb = tf.concat([emb_pad,emb_train, emb_pretrained], axis=0, name="emb")
            ...
            self.width_size2 = 6
            self.width_size3 = 3
            self.width_size_pool1 = 6
            self.width_size_pool2 = 2
            self.stride_pool1 = 2
            self.stride_pool2 = 1
            self.num_filter1 = 225
            self.num_filter2 = 200
            self.num_filter3 = 200
            ...
            pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[self.width_size_pool1, 1], strides=self.stride_pool1)
            pool1_shape = ((self.max_length - self.width_size1 + 1) - self.width_size_pool1 + 1)/self.stride_pool1
            ...
            conv2 = tf.layers.conv2d(inputs=pool1,
                                     ...
                                     kernel_size=[self.width_size2,1],
                                     padding="valid",
                                     activation=tf.nn.relu)
            # Pooling Layer 2
            cov2_shape = pool1_shape - self.width_size2 + 1
            print(cov2_shape)
            print(conv2.get_shape())
            pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[self.width_size_pool2 , 1], strides=self.stride_pool2)
            pool2_shape = (cov2_shape - self.width_size_pool2 + 1)/self.stride_pool2
            print(pool2_shape)
            ...
            conv3 = tf.layers.conv2d(inputs=pool2,
                                     ...
                                     kernel_size=[self.width_size3, 1],
                                     padding="valid",
                                     activation=tf.nn.relu)
            # Pooling Layer 3
            cov3_shape = pool2_shape - self.width_size3 + 1
            print(conv3.get_shape())
            print(cov3_shape)
            pool3 = tf.layers.max_pooling2d(inputs=conv3, pool_size=[cov3_shape, 1], strides=1)
            print(pool3.get_shape())
            self.output_flat = tf.reshape(pool3, [-1,self.num_filter3])
            self.h_drop = tf.nn.dropout(self.output_flat, self.keep_prob)

            # Fully connected layer
            with tf.name_scope("rnn_layer"):
                # Parameters to calibrate
                W = tf.get_variable("W", [self.num_filter3, self.output_size], tf.float32,initializer=tf.contrib.layers.xavier_initializer())
